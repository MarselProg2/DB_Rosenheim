...
def drop_allgemein_columns(pivot: pd.DataFrame) -> pd.DataFrame:
    """Entfernt alle Spalten, die 'Allgemein' oder 'Zubehör' enthalten."""
    if pivot.empty:
        return pivot
    
    # Begriffe, die aus den Spalten entfernt werden sollen
    exclude_terms = ['Allgemein', 'Zubehör']
    
    # Bei MultiIndex-Spalten prüfen, ob ein Ausschluss-Begriff in einem der Levels vorkommt
    cols_to_keep = []
    for col in pivot.columns:
        if isinstance(col, tuple):
            # MultiIndex: Prüfe alle Ebenen
            if not any(term in str(c) for c in col for term in exclude_terms):
                cols_to_keep.append(col)
        else:
            # Einfacher Index
            if not any(term in str(col) for term in exclude_terms):
                cols_to_keep.append(col)
...
def build_all_ebenen_table(df_filtered: pd.DataFrame, ebenen: list[str]) -> pd.DataFrame:
    ...
    # Sortierung: E1, E2, E3 in richtiger Reihenfolge, Total-Zeilen am Ende jeder Ebene
    def sort_key(idx):
        # Ebene extrahieren (E1, E2, E3)
        ...
        is_total = 1 if 'Total' in idx else 0
        
        return (ebene, is_total, idx)
